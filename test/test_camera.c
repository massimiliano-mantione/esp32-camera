...
static void camera_performance_test(uint32_t xclk_freq, uint32_t pic_num)
{
    ...
    for (; format_s <= format_e; format_s++) {
        for (size_t i = 0; i <= max_size; i++) {
            ESP_LOGI(TAG, "\n\n===> Testing format:%s resolution: %d x %d <===", get_cam_format_name(*format_s), resolutions_list[i].width, resolutions_list[i].height);
    ...
    for (size_t i = 0; i <= max_size; i++) {
        printf("%4d x %4d ,     %5.2f,     %6d,      %5.2f,     %7d,      %5.2f,     %7d \n",
               resolutions_list[i].width, resolutions_list[i].height,
