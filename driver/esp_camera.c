...
esp_err_t esp_camera_init(const camera_config_t *config)
{
    ...
    ESP_LOGD(TAG, "Setting frame size to %dx%d", resolutions_list[frame_size].width, resolutions_list[frame_size].height);
...
camera_fb_t *esp_camera_fb_get()
{
    ...
    if (fb) {
        fb->width = resolutions_list[s_state->sensor.status.framesize].width;
        fb->height = resolutions_list[s_state->sensor.status.framesize].height;
