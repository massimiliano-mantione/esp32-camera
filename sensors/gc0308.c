...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    uint16_t row_s = (resolutions_list[FRAMESIZE_VGA].height - h) / 2;
    uint16_t col_s = (resolutions_list[FRAMESIZE_VGA].width - w) / 2;
    (void)row_s;
    (void)col_s;
    ...
    for (size_t i = 0; i < sizeof(subsample_cfgs) / sizeof(struct subsample_cfg); i++) {
        ...
        if ((win_w * cfg->ratio_numerator / cfg->ratio_denominator >= w) && (win_h * cfg->ratio_numerator / cfg->ratio_denominator >= h)) {
            ...
            row_s = (resolutions_list[FRAMESIZE_VGA].height - win_h) / 2;
            col_s = (resolutions_list[FRAMESIZE_VGA].width - win_w) / 2;
